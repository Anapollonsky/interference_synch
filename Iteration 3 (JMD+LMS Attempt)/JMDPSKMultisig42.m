...
tapest  = zeros(taplen, len);
tapest1 = zeros(taplen, len);
tapest2 = zeros(taplen, len);% Tap weight vector
...
minimat1 = zeros(m1, m2, len);
minimat2 = zeros(m1, m2, len);
z1 = zeros(len, 1);
z2 = zeros(len, 1);
weq = zeros(len, 1);
...
for t = 1:len
    ...
    if numsigs(t) > 1
        ...
        if t < (switch12 + taplen)
            ...
            for q = 0:(m1-1) % y1
                tapin1(:, t) = tapin1(:, t) + ...
                    flipud(squeeze(sum(minimat1(q+1, :, t-taplen:t-1), 2) * step(mod1, q)));
            end
            
            for a = 0:(m2-1) %y2
                  tapin2(:, t) = tapin2(:, t) + ...
                    flipud(squeeze(sum(minimat2(:, a+1, t-taplen:t-1), 1) * step(mod2, a)));  
            end
        end
%         err(t) = 1*(y1(t-1) + y2(t-1)) - tapest(:, t)'*tapin1(:, t) - tapest(:, t)'*tapin2(:, t);
%         tapest(:, t+1) = tapest(:, t) + lstep * err(t)' * (tapin1(:, t)+tapin2(:, t));
        err1(t) = y1(t-1) - tapest1(:, t)'*tapin1(:, t);
        err2(t) = y2(t-1) - tapest2(:, t)'*tapin2(:, t);
%         tapest(:, t+1) = tapest(:, t) + lstep * err1(t)' * tapin1(:, t) + lstep * err2(t)' * tapin2(:, t);
        err(t) = err1(t) + err2(t);
        tapest1(:, t+1) = tapest1(:, t) + lstep * err1(t)' * tapin1(:, t);
        tapest2(:, t+1) = tapest2(:, t) + lstep * err2(t)' * tapin2(:, t);
        weq1(t) = sum(tapest1(:, t)' * w(t));
        weq2(t) = sum(tapest2(:, t)' * w(t));
        for q = 0:(m1-1) % For every possible y1
            for a = 0:(m2-1) %For every possible y2
                minimat1(q+1, a+1, t) = ...
                    abs(weq1(t) - g1*step(mod1, q) - g2*step(mod2, a));
            end
        end
        [r, c, v] = find(minimat1(:, :, t) == min(min(minimat1(:, :, t)))); % Find minimum
        minimat1(:, :, t) = 1./(minimat1(:, :, t).^2);
        minimat1(:, :, t) = minimat1(:, :, t)/sum(sum(minimat1(:, :, t)));
        z1(t) = r(1)-1;
%         z2(t) = c(1)-1;
        ...
        for q = 0:(m1-1) % For every possible y1
            for a = 0:(m2-1) %For every possible y2
                minimat2(q+1, a+1, t) = ...
                    abs(weq2(t) - g1*step(mod1, q) - g2*step(mod2, a));
            end
        end
        [r, c, v] = find(minimat2(:, :, t) == min(min(minimat2(:, :, t)))); % Find minimum
        minimat2(:, :, t) = 1./(minimat2(:, :, t).^2);
        minimat2(:, :, t) = minimat2(:, :, t)/sum(sum(minimat2(:, :, t)));
        z2(t) = c(1)-1;
    end
